...
const DELETE_ACCOUNT_META: Record<
  SupportedLocale,
  { title: string; description: string; backToHome: string; markdownFile: string }
> = {
  es: {
    title: "Eliminar cuenta | Finnon",
    description:
      "Instrucciones para solicitar la eliminación de tu cuenta de Finnon y conocer qué ocurre con tus datos.",
    backToHome: "Volver al inicio",
    markdownFile: "delete-account-es.md",
  },
  en: {
    title: "Delete Account | Finnon",
    description:
      "Instructions to request deletion of your Finnon account and understand what happens to your data.",
    backToHome: "Back to home",
    markdownFile: "delete-account-en.md",
  },
};
...
async function resolveDeleteAccountMarkdownPath(markdownFile: string): Promise<string> {
  const candidates = [
  ...
  throw new Error(`Delete account markdown file not found: ${markdownFile}`);
}

async function readDeleteAccountMarkdown(locale: SupportedLocale): Promise<string> {
  const markdownFile = DELETE_ACCOUNT_META[locale].markdownFile;
  const filePath = await resolveDeleteAccountMarkdownPath(markdownFile);
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const meta = DELETE_ACCOUNT_META[locale];
...
export default async function DeleteAccountPage() {
  const locale = normalizeLocale(await getLocale());
  const meta = DELETE_ACCOUNT_META[locale];
  const markdown = await readDeleteAccountMarkdown(locale);
